...
def main():
    ...
    (function() {
        ...
        function detectFoldable() {
            ...
            // 外屏: 窄且长
            if (width < 800 && ratio < 0.5) {
                document.body.classList.add('foldable-outer');
                document.body.classList.remove('foldable-inner');
                console.log('📱 折叠屏外屏模式');
            }
            // 内屏: 宽且接近正方形
            else if (width >= 800 && width <= 1400 && ratio >= 0.8 && ratio <= 1.2) {
                document.body.classList.add('foldable-inner');
                document.body.classList.remove('foldable-outer');
                console.log('📱 折叠屏内屏模式');
            }
            // 普通屏幕
            else {
                document.body.classList.remove('foldable-outer', 'foldable-inner');
            }
        }
