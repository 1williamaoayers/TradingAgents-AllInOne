...
"""
    ...
    def get_stock_news_unified(
        ...
        try:
            ...
            # 使用现有Toolkit类初始化分析器
            # 注意：UnifiedNewsAnalyzer 使用 toolkit 来调用其他基础工具
            # 这里传入 Toolkit 类本身，因为 Toolkit 中的工具方法都是静态的
            analyzer = UnifiedNewsAnalyzer(Toolkit)
            
            # 调用统一新闻接口
            logger.info(f"📰 [统一新闻工具] 委托给 UnifiedNewsAnalyzer 处理: {ticker}")
            result = analyzer.get_stock_news_unified(stock_code=ticker, max_news=10)
            
            content = result.get('content', '')
            if not content:
                logger.warning(f"⚠️ [统一新闻工具] UnifiedNewsAnalyzer 返回空内容")
                return "未找到相关新闻"
