...
def create_news_analyst(llm, toolkit):
    ...
    def news_analyst_node(state):
        ...
        # 创建统一新闻工具 (解包元组：create_unified_news_tool 返回 (news_tool, sentiment_tool))
        unified_news_tool, _ = create_unified_news_tool(toolkit)
...
---
*数据来源：[列出所有使用的数据源，如AKShare/RSS/Serper等]（截至{current_date}）*
*报告生成时间：{current_date}*

3. **重要**: 此脚注必须出现在报告的最末尾，使用斜体格式，数据源名称需准确反映工具返回的内容。

请撰写详细的中文分析报告，并在报告末尾附上Markdown表格总结关键发现，然后添加标准脚注。"""
        )
        ...
        if ('DashScope' in llm.__class__.__name__ 
            ...
            try:
                ...
                if isinstance(pre_fetched_news_result, dict):
                    pre_fetched_news = pre_fetched_news_result.get("content", "")
                else:
                    pre_fetched_news = str(pre_fetched_news_result)
...
🏁 **报告脚注要求（重要）：**
工具提供的名为【最新新闻数据】的内容末尾已包含标准化的数据源脚注。
请在生成的报告最后，**原样保留**该脚注。
若工具输出未包含脚注，请勿自行添加。"""
            ...
            if current_tool_calls == 0:
                ...
                try:
                    ...
                    if isinstance(forced_news_result, dict):
                        ...
                        forced_news = str(forced_news_result)
