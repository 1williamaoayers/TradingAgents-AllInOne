...
## 功能发布
- **动作**: 推送代码触发构建。
- **新增**: 在自选股管理页面增加了 "🔄 同步股票名称" 按钮。
- **意义**: 任何部署此镜像的用户，都可以一键更新最新的代码-名称映射，无需运行脚本。

## 验证结论
- **构建状态**: GitHub Actions 构建成功 (ID: 21254848072)。
- **部署状态**: `docker-compose pull` 成功拉取新镜像。
- **功能验证**: 页面已出现 "同步股票名称" 按钮，点击即可运行后台脚本。全流程闭环。
