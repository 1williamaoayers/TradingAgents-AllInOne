...
- [ ] 我是否使用了 `Invoke-WebRequest`？ → **严禁**，请用 `src/scripts/check_port.py`
...
- [ ] 我是否建议用户创建测试脚本？ → **绝对禁止**
- [ ] 我是否使用了现有的测试脚本？ → **绝对禁止**
- [ ] **禁止测试脚本锁 (NO TEST SCRIPT LOCK)**: 严禁创建临时测试脚本 (verify_*.py) 作为交付物或验证条件。
- [ ] 正确做法：直接运行项目本身，查看实际运行结果

**⚠️ 任何形式的测试脚本都是违规行为！必须基于真实环境 (Docker/App) 的直接指征！**
